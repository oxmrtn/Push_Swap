...
/*   Updated: 2024/07/23 14:12:52 by mtrullar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    execute_moov(char *str, t_DLIST **stackA, t_DLIST **stackB)
{
    ...
    while (moov[i])
    {
        if (!ft_strncmp(moov[i], "pb", ft_strlen(moov[i])))
            ...
            ft_rr(stackA, stackB);
        else if (!ft_strncmp(moov[i], "revt", ft_strlen(moov[i])))
            ft_rrr(stackA, stackB);
        else if (!ft_strncmp(moov[i], "ra", ft_strlen(moov[i])))
            ft_rotate(stackA);
        else if (!ft_strncmp(moov[i], "reva", ft_strlen(moov[i])))
            ft_reverse_rotate(stackA);
        else if (!ft_strncmp(moov[i], "rb", ft_strlen(moov[i])))
            ft_rotate(stackB);
        else if (!ft_strncmp(moov[i], "revb", ft_strlen(moov[i])))
